...
{
	global $request;
	change_skin_by_name($request['skin'],$request['type']);
	redirect('?m=system&s=changeskin');
}
//-------------------------------------修改模板-----------------------------------------------------//
/**
...
function get_skins_info($tArr)
{
	...
	{
		if(STYLENAME==$entry['path'])
		{
			echo '<dl class="u_box">';
		}
		elseif(MOBILENAME==$entry['path'])
		{
			echo '<dl class="u_box">';
		}
		else
		...
		echo '<dd><a href="./index.php?m=system&s=changeskin&a=editSkin&skin='.$entry['path'].'" class="creatbt" onmouseover="ddrivetip(\'点击编辑此模版\', \'\', 90)" onmouseout="hideddrivetip()">编辑模版</a>&nbsp;<a href="?m=system&s=changeskin&a=delete&skinname='.$entry['path'].'" class="creatbt" onmouseover="ddrivetip(\'点击删除此模板\', \'\', 90)" onclick="return confirm(\'您确认要删除本模板?\n一旦删除，此模板将不可恢复。\');">删除模版</a><a href="./index.php?m=system&s=changeskin&a=changeskin&skin='.$entry['path'].'&type=pc" class="creatbt" onmouseover="ddrivetip(\'点击使用此模版作为PC站模版\', \'\', 90)" onmouseout="hideddrivetip()">应用PC站</a>&nbsp;<a href="./index.php?m=system&s=changeskin&a=changeskin&skin='.$entry['path'].'&type=mobile" class="creatbt" onmouseover="ddrivetip(\'点击使用此模版作为移动站模版\', \'\', 90)" onmouseout="hideddrivetip()">应用移动站</a></dd>';
		echo '</dl>';
	}
}
