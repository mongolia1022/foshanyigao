...
function sys_href($channelId=0,$type='article',$id=0,$action=0)
{
	...
	if($type=='user')
	{
		$rs = $db->get_row("SELECT * FROM ".TB_PREFIX."menu WHERE type ='user' AND dtLanguage = '".$_SESSION[TB_PREFIX.'doclang']."' LIMIT 1");
		if($rs)
		$channelId = $rs->id;
		else
	...
	switch ($type)
	{
		...
		case 'comment':       //评论模块
			...
			$link = URLREWRITE?'/'.$menuName.'/view_comment/r-'.$id.'_'.$action.'.html':'./?p='.$channelId.'&a=view_comment&r='.$id.'&comment_mdtp='.$action;
			break;
		case 'submitcomment':       //评论模块 提交评论 action
		    $id = $id?$id:0;
			$link = URLREWRITE?'/'.$menuName.'/submitcomment/r-'.$id.'_'.$action.'.html':'./?p='.$channelId.'&a=submitcomment&r='.$id.'&comment_mdtp='.$action;
			break;
		case 'destroycomment':       //评论模块 删除评论 action
		    $id = $id?$id:0;
			$action = $action?$action:0;
			$link = URLREWRITE?'/'.$menuName.'/destroycomment/comment-'.$id.'-comment_mdtp-'.$action.'.html':'./?p='.$channelId.'&a=destroycomment&comment='.$id.'&comment_mdtp='.$action;
			break;
		case 'auditingcomment':       //评论模块 审核评论 action
		    $id = $id?$id:0;
			$action = $action?$action:0;
			$link = URLREWRITE?'/'.$menuName.'/auditingcomment/comment-'.$id.'-comment_mdtp-'.$action.'.html':'./?p='.$channelId.'&a=auditingcomment&comment='.$id.'&comment_mdtp='.$action;
			break;
		case 'list':        //文章列表
			$link = URLREWRITE?'/'.$menuName.'/r-'.$id.'.html':'./?p='.$channelId.'&a=view&r='.$id;
			break;
		case 'download':    //下载模块
			$link = URLREWRITE?'/'.$menuName.'/download/r-'.$id.'.html':'./?p='.$channelId.'&a=download&r='.$id;
			break;
		case 'form_action': //表单提交
			$link = URLREWRITE?'/'.$menuName.'/create/':'./?p='.$channelId.'&a=create';
			break;
		case 'guestbook':    //留言模块
			$link = URLREWRITE?'/'.$menuName.'/r-'.$id.'.html':'./?p='.$channelId.'&a=view&r='.$id;
			break;
		...
		case 'job_send':  //招聘模块 表单提交
			$link = URLREWRITE?'/'.$menuName.'/send/r-'.$id.'.html':'./?p='.$channelId.'&a=send&r='.$id;
			break;
		case 'picture':       //图片模块
			$link = URLREWRITE?'/'.$menuName.'/r-'.$id.'.html':'./?p='.$channelId.'&a=view&r='.$id;
			break;
		...
		case 'product_basket': //查看购物车
			$link = URLREWRITE?'/'.$menuName.'/basket/':'./?p='.$channelId.'&a=basket';
			break;
		case 'product_basketqr':  //购物车数据确认修改
			$link = URLREWRITE?'/'.$menuName.'/basketqr/':'./?p='.$channelId.'&a=basketqr';
			break;
		case 'product_intobasket':  //购物车加入产品
			$link = URLREWRITE?'/'.$menuName.'/intobasket/r-'.$id.'.html':'./?p='.$channelId.'&a=intobasket&r='.$id;
			break;
		case 'product_basket_submit': //提交购物车
			$link = URLREWRITE?'/'.$menuName.'/submitbasket/':'./?p='.$channelId.'&a=submitbasket';
			break;
		case 'rss':    //RSS
			$link = URLREWRITE?'/'.$menuName.'/get_rss/r-'.$id.'-i-'.$action.'.html':'./?p='.$channelId.'&a=get_rss&r='.$id.'$i='.$action;
			break;
		case 'user':     //会员中心
		$id=$id?$id:'index';
		$link = URLREWRITE?'/'.$menuName.'/'.$id.'/':'./?p='.$channelId.'&a='.$id;
			break;
		case 'user_pay':     //会员中心  订单支付
		$link = URLREWRITE?'/'.$menuName.'/gotopay/r-'.$id.'.html':'./?p='.$channelId.'&a=gotopay&r='.$id;
			break;	
		case 'pay':  //在线支付
		if(empty($channelId))$link = URLREWRITE?'/pay/':'./?m=pay';else $link = URLREWRITE?'/pay/'.$channelId.'/':'./?m=pay&a='.$channelId;
			break;
		case 'search':  //搜索
		$link = URLREWRITE?'/search/':'./?m=search';
			break;
		default:
		$link = URLREWRITE?'/'.$menuName.'/'.$type.'/':'./?p='.$channelId.'&a='.$type;
	}
	return $link;
}
//获取SESSION
