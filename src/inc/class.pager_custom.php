...
class Pager
{
	...
	public function Show($url,$style=0)
	{
		...
		elseif($style==1)
		{
			...
			$tempStr ='<div class="page_m">';
			$tempStr .=$this->prvNo()<1?'':'<div class="page_o"><a href="'.$this->rootpath.$url.$this->prvNo().$this->anchor.'"><img src="/inc/img/pager/page_o.png"></a></div>　　';
			
			$tempStr .=$tstart>$this->fristNo()?'<div class="page_l"><a href="'.$this->rootpath.$url.$this->fristNo().$this->anchor.'">1</a></div>':'';
			$tempStr .=$tstart>$this->fristNo()+1?'<div class="page_c">...</div>':'';
			for($i=$tstart;$i<$tend+1;$i++)
			{
				$tempStr .='<div class="page_'.($i==$this->fristNo()?'l':($i==$this->lastNo()?'r':'c')).'"><a href="'.$this->rootpath.$url.$i.$this->anchor.'" '.($this->cPage==$i?'class="now"':'').'>'.$i.'</a></div>';
			}
			$tempStr .= $tend<$this->lastNo()-1?'<div class="page_c">...</div>':'';
			$tempStr .= $tend<$this->lastNo()?'<div class="page_r">'.$this->lastNo().'</div>':'';
			
			$tempStr .=	$this->nextNo()>$this->totalPageNo?'':'　　<div class="page_n"><a href="'.$this->rootpath.$url.$this->nextNo().$this->anchor.'"><img src="/inc/img/pager/page_n.png"></a></div>';
			$tempStr .='</div>';
		}
		elseif($style==2)
		{
		...
		{
			$tempStr =	$this->prvNo()<1?'<a href="javascript:;"><< 前一页</a> ':'<a href="'.$this->rootpath.$url.$this->prvNo().$this->anchor.'"><< 前一页</a> ';
			$tempStr .=	$this->nextNo()>$this->totalPageNo?'<a href="javascript:;">下一页 >></a>':'<a href="'.$this->rootpath.$url.$this->nextNo().$this->anchor.'">下一页 >></a>';
		}
	...
	public function enShow($url,$style=0)
	{
		...
		elseif($style==1)
		{
			...
			$tempStr .=	$this->prvNo()<1?'<li><a href="javascript:;">Previous Page</a></li>':'<li><a href="'.$this->rootpath.$url.$this->anchor.'">Previous Page</a></li>';
			if($this->cPage<$tpageNum/2)
			{
				$tstart=1;
				$tend=$tpageNum+1;
			...
			$tempStr .=	$this->nextNo()>$this->totalPageNo?'<li><a href="javascript:;">Next Page</a></li>':'<li><a href="'.$this->rootpath.$url.$this->nextNo().$this->anchor.'">Next Page</a></li>';
			$tempStr .='</ul>';
		}
		elseif($style==2)
		{
		...
		{
			$tempStr =	$this->prvNo()<1?'<a href="javascript:;"><< Previous Page</a> ':'<a href="'.$this->rootpath.$url.$this->prvNo().$this->anchor.'"><< Previous Page</a> ';
			$tempStr .=	$this->nextNo()>$this->totalPageNo?'<a href="javascript:;">Next Page >></a>':'<a href="'.$this->rootpath.$url.$this->nextNo().$this->anchor.'">Next Page >></a>';
		}
...
class sqlbuilder
{
	...
	function __construct($name,$sql,$order,$db,$pagesize=10,$paging=true,$rootpath='./index.php',$anchor='')
	{
		...
		$torder = $this->get_str($request[$name.'o']);
		$tpage = intval($this->get_str($request[$name.'p']));//xss 
	...
	{
		return $this->pager->Show($this->build_url(),PAGESTYLE);
	}
	public function get_en_pager_show()
	{
		return $this->pager->enShow($this->build_url(),PAGESTYLE);
	}
