<?php
//检查图片是否存在 grysoft/
function ispic($picUrl,$i=0,$nopicUrl="/inc/img/system/nopic.jpg")
{
	$tempUrl = explode('|',$picUrl);
	$tempPic = explode('/',$tempUrl[$i]); //分割图片地址信息
	if($tempPic[$i]!='http:')
	{
		if(is_file(ABSPATH.$tempUrl[$i]))
		return get_root_path().$tempUrl[$i];
		else
